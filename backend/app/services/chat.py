...
from app.logging_context import bound_turn
...
async def run_turn(
    ...
    **`is_admin` is never set from here** — R-54 requires ownership at the route, so
    ...
    # **Correlation, bound once for the whole turn** (T-604, R-79(2)). Every event the
    # run emits downstream — `rag.router.*`, `rag.rerank.*`, `security.injection.*`,
    # `rag.search.*`, `graph.node_failed` — named its own subject and nothing else, so the
    # log stream could say a router call was slow and never which turn it belonged to.
    # Binding here rather than in a node covers the denial path too (`govern` refuses
    # before `telemetry_start` runs), and the key is `(conversation_id, turn_index)`
    # because that pair is stable across a resume and is exactly what the durable
    # `turn_telemetry` row carries — a freshly minted run id would be regenerated by the
...
def record_feedback(
    ...
    **The consumer exists now** (T-609, R-80): `tools.feedback_calibration` reads the thumb
    from `messages.feedback` and joins it to the T-604 telemetry row. This event stays for the
    *stream* — a stable name with a fixed key set that an external sink can bind — and R-80(2)
    is why it is not more than that: feedback is a **measurement, not a controller**, so
    nothing consumes this event to change a threshold automatically.
    ...
    **Ids and the enum value only** — R-43(5)'s "no payload text, ever". Like the telemetry
    ...
    their rating" and "the user never rated" are different facts, and the calibration report
    is precisely the consumer that has to tell them apart — a cleared rating is not evidence
    either way, while a missing one is not evidence at all.
